        ...
        for row in rows:
            ...
            if row.previous_hash != expected_previous:
                return ChainIntegrityOut(
                    ...
                    failed_index=row.index,
                    detail=(
                        f"Block {row.index} previous_hash does not match "
                        "the prior block hash."
                    ),
                )
            if row.block_hash != recomputed:
                return ChainIntegrityOut(
                    ...
                    checked_blocks=len(rows),
                    failed_index=row.index,
                    detail=f"Block {row.index} hash does not match its payload.",
                )
            ...
            if payload.get("previous_hash") != row.previous_hash:
                return ChainIntegrityOut(
                    ...
                    checked_blocks=len(rows),
                    failed_index=row.index,
                    detail=f"Block {row.index} payload previous_hash mismatch.",
                )
