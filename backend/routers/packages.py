...
def get_packages(request: Request, session: Session = Depends(get_db)) -> list[PackageOut]:
    return package_service.list_packages(session, request)
...
def create_package(
    ...
    return package_service.create_package(session, payload, request)


@router.get("/packages/{package_id}", response_model=PackageOut)
def get_package(package_id: str, request: Request, session: Session = Depends(get_db)) -> PackageOut:
    return package_service.get_package(session, package_id, request)
...
def get_package_qr(package_id: str, request: Request, session: Session = Depends(get_db)) -> FileResponse:
    package = package_service.get_package(session, package_id, request)
